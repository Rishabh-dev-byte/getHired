...
const JobPage = () => {
  ...
  const { data: job, fn: fnJobs } = useFetch(getSinglejobs, {
    job_id: id,
  });
  useEffect(() => {
    fnJobs();
  ...
  const { data: hiring, fn: fnstatus } = useFetch(updateHiringStatus, {
    job_id: id,
  });
    ...
    <div className="flex flex-col">
      ...
      {job?.applications.length > 0 && job?.recruiter_id === user?.id && (
        <div className="flex flex-col gap-2">
          <h2 className="font-bold mb-4 text-xl ml-1 pl-5 mt-5">
            Applications
          </h2>
